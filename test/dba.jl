importall PostgreSQL

function test_dbi()
    println("Testing class membership for Postgres type")
    @test Postgres <: DBI.DatabaseSystem
    println("Test passes")
    

    println("Testing output of connect")
    conn = connect(Postgres, "localhost", "postgres")

    @test isa(conn, DBI.DatabaseHandle)
    @test isdefined(conn, :status)
    println("Connect worked as expected")
    println("Testing execution of a statement")
    ...
    println("Statement results as expected")

    println("Testing DataArray output")
    ...
    println("DataArray results successful")

    finish(stmt)
    println("Running test to create table, insert data, and get resulting data")
    ...
    finish(stmt)
    println("Persistence passed")

    println("Testing string escaping")
